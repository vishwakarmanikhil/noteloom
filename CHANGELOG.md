...
- `examples/lan-collab/`: two brand-new tabs joining the same room at nearly the same moment would deadlock forever — each discovered the other via the relay's roster before either's "am I alone" seed timeout fired, so neither ever seeded a starter document, and since both stayed empty there was nothing for either side's `syncRequest`/`syncResponse` handshake to actually deliver. Fixed with a deterministic tie-break (whichever peerId sorts lower seeds, decided independently and synchronously by each side, same idea as the existing WebRTC `initiator` tie-break) applied *before* the connection's own initial sync handshake runs, so that handshake picks up real content naturally instead of two empty snapshots meeting in the middle. Found and fixed while verifying the new live-cursor/auto-reconnect-watchdog example code in a real two-tab browser session.
