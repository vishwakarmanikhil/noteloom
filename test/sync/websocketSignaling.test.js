...
describe('createWebSocketSignaling', () => {
  ...
  it('appends query params correctly when the url already has one', () => {
    createWebSocketSignaling({
      url: 'ws://host:8080/path?token=abc',
      roomId: 'doc-1',
      peerId: 'peer-a',
      WebSocketImpl: FakeWebSocket,
    });
  ...
  it('sends immediately once already open', () => {
    ...
    expect(socket.sentMessages).toEqual([
      JSON.stringify({ type: 'signal', toPeerId: 'peer-b', payload: { kind: 'answer', sdp: 'z' } }),
    ]);
  });
