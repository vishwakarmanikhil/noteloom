...
describe('syncProtocol', () => {
  ...
  it('round-trips an op message carrying an arbitrary envelope', () => {
    const envelope = {
      ...
      slot: {
        ...
        originId: null,
        peerId: 'a',
        clock: { wallTime: 1, counter: 0, peerId: 'a' },
        deleted: false,
      },
      subtree: { blocks: [], runs: [] },
    };
  ...
  it('round-trips syncRequest/syncResponse', () => {
    expect(decodeMessage(encodeMessage(syncRequestMessage()))).toEqual({
      type: MESSAGE_TYPE.SYNC_REQUEST,
    });
    const doc = { blocks: [], runs: [], rootId: 'root', fieldTypes: [] };
    expect(decodeMessage(encodeMessage(syncResponseMessage(doc)))).toEqual({
